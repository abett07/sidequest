...
import {
  ...
  flushOutbox, rpcCompleteQuest, rpcCompleteFocusSession, rpcUnlockSkill,
} from '../data/sync';
  ...
  persist(
    (set, get) => ({
      ...
      unlockSkillNode: (nodeId) => {
        ...
        // Sync to backend with idempotency mutation id.
        const userId = get().user.id;
        if (userId) {
          const mutationId = `ms_${node.key}_${new Date().toISOString()}`;
          void rpcUnlockSkill(node.key, mutationId).catch(() => {
