...
function isCoalescibleWrite(entry: { table: string; operation: 'upsert' | 'update' | 'delete' }): boolean {
  if (entry.operation === 'update') return true;
  // Keep append-only event tables as discrete writes.
  if (entry.operation === 'upsert') {
    return !['state_checkins', 'focus_sessions', 'battle_log', 'narrative_events'].includes(entry.table);
  }
  return false;
}
...
function pushOutbox(entry: Omit<OutboxEntry, 'id' | 'createdAt'> & { dedupeKey?: string }): string {
  ...
  if (targetId !== undefined && isCoalescibleWrite(entry)) {
...
export async function rpcUnlockSkill(
  nodeKey: string, clientMutationId?: string,
): Promise<{ unlocked?: string; stat_bonus?: Record<string, number>; already_unlocked?: boolean; error?: string }> {
  try {
...
export async function rpcRedeemReward(
  rewardId: string,
  clientMutationId?: string,
): Promise<{ reward_id?: string; coins_remaining?: number; already_redeemed?: boolean; error?: string }> {
  try {
